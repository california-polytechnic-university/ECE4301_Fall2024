...
#include "xtmrctr.h"
#include "crypto_aead.h"  // Include Xoodyak AEAD headers
...
void test_case(const char *test_name, uint8_t *plaintext, size_t plaintext_len, uint8_t *associated_data, size_t ad_len,
    ...
    unsigned long long ciphertext_len = 0; // Updated to match API requirements
    ...
    unsigned long long decrypted_len = 0; // Updated to match API requirements
...
int main() {
    ...
    xil_printf("=== Xoodyak AEAD Encryption/Decryption Tests ===\n\r");
    ...
    while (1) {
        char choice_str[10];
        read_choice(choice_str, sizeof(choice_str));
