...
#include "xtmrctr.h"
#include "crypto_aead.h"  // Include ASCON AEAD headers
...
void test_case(const char *test_name, uint8_t *plaintext, size_t plaintext_len, uint8_t *associated_data, size_t ad_len,
    ...
    unsigned long long ciphertext_len = 0; // Match the expected type
    ...
    unsigned long long decrypted_len = 0; // Match the expected type

    int result = crypto_aead_decrypt(decrypted_text, &decrypted_len, NULL, ciphertext, ciphertext_len,
                                     associated_data, ad_len, nonce, key);

    XTmrCtr_Stop(&TimerInstance, 0);
    u64 decryption_cycles = XTmrCtr_GetValue(&TimerInstance, 0);
...
int main() {
    ...
    xil_printf("=== ASCON AEAD Encryption/Decryption Tests ===\n\r");
    ...
    while (1) {
        char choice_str[10];
        xil_printf("\n\rEnter a test case number (1-5) or 0 to exit:\n\r");
        read_choice(choice_str, sizeof(choice_str)); // Fetch user input
        int choice = atoi(choice_str); // Convert to integer
        ...
        if (choice >= 1 && choice <= 5) {
            run_test_case(choice); // Execute selected test case
        } else {
            xil_printf("Invalid input. Please enter a valid number (1-5).\n\r");
        }
    }
