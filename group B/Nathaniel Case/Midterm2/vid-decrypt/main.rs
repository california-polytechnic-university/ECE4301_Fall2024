...
fn main() {
    ...
    let mut stream = TcpStream::connect("192.168.0.171:7878").unwrap();
    ...
    match cipher.decrypt(&nonce, &video_buffer[..]) {
        Ok(plaintext) => {
        ...
        Err(e) => {
            ...
            // Optionally, handle failure (e.g., retry, exit, or alert the user)
        }
    }
