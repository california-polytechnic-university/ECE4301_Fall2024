use std::io::{stdin, stdout, Write, Read, Result};
use std::fs;
...
fn handle_client(mut stream: TcpStream) {
    ...
    let video_file: Vec<u8> = fs::read("output.h264").unwrap(); // Read the H264 output file
...
fn get_video() {
    // Spawns child libcamera-vid process to save camera video
    let mut child = Command::new("libcamera-vid")
        .args([
            "--output", "output.h264",   // Output file for the video (H264 format)
            "--width", "640",            // Video width
            "--height", "480",           // Video height
            "--framerate", "30",         // Frame rate
            "--inline",                  // H264 inline headers (optional)
        ])
        .spawn()
        .unwrap();
